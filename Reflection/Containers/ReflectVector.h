...
namespace DNVS { namespace MoFa { namespace Reflection {
    template<typename T>
    struct AutoReflectImplemented<T, std::enable_if_t<TypeUtilities::IsVector<T>>>
    {
		...
		static void Reflect(const TypeLibraries::TypeLibraryPointer& typeLibrary)
		{
			...
			Class<Container, IgnoreAutoReflector, std::unique_ptr<Container>> cls(typeLibrary, "__Vector__");

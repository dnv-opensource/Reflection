...
namespace DNVS { namespace MoFa { namespace Reflection {
    template<typename T>
    struct AutoReflectImplemented<T, std::enable_if_t<TypeUtilities::IsMap<T>>>
    {
		...
		static void Reflect(const TypeLibraries::TypeLibraryPointer& typeLibrary)
		{
			...
			Class<Container, IgnoreAutoReflector, std::unique_ptr<Container>> cls(typeLibrary, "__Map__");
			cls.AddAttribute<ContainerAttribute>(Types::TypeId<typename Container::value_type>(), true);
			cls.Constructor();
	...
	template<typename T>
	struct AutoReflectImplemented<T, std::enable_if_t<TypeUtilities::IsMultiMap<T>>>
	{
		static const bool value = true;
		static void Reflect(const TypeLibraries::TypeLibraryPointer& typeLibrary)
		{
			...
			Class<Container, IgnoreAutoReflector, std::unique_ptr<Container>> cls(typeLibrary, "__MultiMap__");
			cls.AddAttribute<ContainerAttribute>(Types::TypeId<typename Container::value_type>(), true);
			cls.Constructor();
            Operators::ConditionalOperator<TypeUtilities::HasEqualTo<value_type>>::Operator(cls, This.Const == This.Const);
            Operators::ConditionalOperator<TypeUtilities::HasNotEqualTo<value_type>>::Operator(cls, This.Const != This.Const);

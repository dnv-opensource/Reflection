...
namespace DNVS { namespace MoFa { namespace Reflection { namespace Contents { 
    class CollectionContent : public IContent 
    {
    public:
        CollectionContent(const std::vector<std::shared_ptr<IContent>>& contentVector);
    ...
    private:
        std::shared_ptr<IContent> Transform(const std::function<std::shared_ptr<IContent>(const std::shared_ptr<IContent>& content)>& algorithm) const;
        std::vector<std::shared_ptr<IContent>> m_contentVector;
    };
} } } }